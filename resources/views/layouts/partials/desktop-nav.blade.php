    ...
    <div class="glass-nav border-b border-slate-200/80 shadow-[0_4px_25px_-5px_rgba(15,23,42,0.06)]">
        <div class="max-w-[1400px] mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-6 shrink-0">
                <a href="{{ auth()->user()->role === 'reporter' ? route('reporter.news.index') : route('news.index') }}" class="flex items-center gap-3 group">
                    ...
                    <div class="flex flex-col">
                        ...
                        <span class="text-[10px] font-extrabold text-slate-400 tracking-wider uppercase mt-0.5 flex items-center gap-1">
            ...
            @auth
            <div class="flex items-center overflow-x-auto custom-scrollbar max-w-full">
                ...
                @else
                    <div class="flex items-center bg-slate-100/90 p-1.5 rounded-2xl border border-slate-200/80 gap-1 shadow-inner">
                        {{-- 1. Feed --}}
                        <a href="{{ route('news.index') }}" class="flex items-center gap-2 px-3.5 py-1.5 rounded-xl text-xs font-extrabold transition-all duration-200 {{ request()->routeIs('news.index') ? 'bg-white text-indigo-600 shadow-md border border-slate-200/60 scale-[1.02]' : 'text-slate-700 hover:text-indigo-600 hover:bg-white/60' }}">
                            <i class="fa-solid fa-newspaper text-indigo-500"></i> Feed
                        </a>

                        {{-- 2. Published --}}
                        <a href="{{ route('news.published') }}" class="flex items-center gap-2 px-3.5 py-1.5 rounded-xl text-xs font-extrabold transition-all duration-200 {{ request()->routeIs('news.published') ? 'bg-white text-indigo-600 shadow-md border border-slate-200/60 scale-[1.02]' : 'text-slate-700 hover:text-indigo-600 hover:bg-white/60' }}">
                            <i class="fa-solid fa-circle-check text-emerald-500"></i> Published
                        </a>
                        
                        {{-- 3. Create --}}
                        @if(auth()->user()->hasPermission('can_direct_publish'))
                        <a href="{{ route('news.create') }}" class="flex items-center gap-2 px-3.5 py-1.5 rounded-xl text-xs font-extrabold transition-all duration-200 {{ request()->routeIs('news.create') ? 'bg-white text-indigo-600 shadow-md border border-slate-200/60 scale-[1.02]' : 'text-slate-700 hover:text-indigo-600 hover:bg-white/60' }}">
                        ...
                        <a href="{{ route('news.drafts') }}" class="flex items-center gap-2 px-3.5 py-1.5 rounded-xl text-xs font-extrabold transition-all duration-200 {{ request()->routeIs('news.drafts') ? 'bg-white text-indigo-600 shadow-md border border-slate-200/60 scale-[1.02]' : 'text-slate-700 hover:text-indigo-600 hover:bg-white/60' }}">
                        ...
                        <a href="{{ route('websites.index') }}" class="flex items-center gap-2 px-3.5 py-1.5 rounded-xl text-xs font-extrabold transition-all duration-200 {{ request()->routeIs('websites.*') ? 'bg-white text-indigo-600 shadow-md border border-slate-200/60 scale-[1.02]' : 'text-slate-700 hover:text-indigo-600 hover:bg-white/60' }}">
                        ...
                        <a href="{{ route('admin.templates.index') }}" class="flex items-center gap-2 px-3.5 py-1.5 rounded-xl text-xs font-extrabold transition-all duration-200 {{ request()->routeIs('admin.templates.*') ? 'bg-white text-indigo-600 shadow-md border border-slate-200/60 scale-[1.02]' : 'text-slate-700 hover:text-indigo-600 hover:bg-white/60' }}">
            ...
            <div class="flex items-center gap-3 shrink-0">
                @auth
                    ...
                    <div class="hidden xl:flex items-center gap-3 border-r border-slate-200 pr-3">
                        ...
                        @if(auth()->user()->role !== 'reporter')
                        <a href="{{ route('credits.index') }}" class="bg-amber-50 hover:bg-amber-100 text-amber-800 px-3.5 py-1.5 rounded-xl text-xs font-extrabold border border-amber-200/80 transition-all shadow-sm flex items-center gap-1.5 hover:scale-105 transform">
                    ...
                    <div class="relative" x-data="{ open: false }">
                        <button @click="open = !open" class="flex items-center gap-2 p-1.5 rounded-2xl hover:bg-slate-100 border border-transparent hover:border-slate-200 transition-all">
                        ...
                        <div x-show="open" @click.away="open = false" x-transition class="absolute right-0 mt-2 w-64 bg-white rounded-3xl shadow-2xl border border-slate-200/90 py-2.5 z-50">
